	...
	async findAll(
		...
		try {
			...
			if (filterDto.warn) {
				...
				else match += ` AND "type" = 'warn'`;
			} else if (filterDto.error) match += ` AND "type" = 'error'`;
			else match += ` AND "type" != 'error' AND "type" != 'warn'`;
			if (filterDto.login && filterDto.login != "")
				match += ` AND "login" = '${filterDto.login}'`;
			if (filterDto.isAdmin) match += ` AND "isAdmin" = 't'`;
			match += ` ORDER BY "date" ${filterDto.asc ? "ASC" : "DESC"} `;
			match += `; `;
			...
			this.logger.error(
				`Failed -> Get all filtered logs on database(${error})`,
				requestMaker,
				true
			);
			throw error;
