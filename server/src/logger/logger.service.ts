...
export class LoggerService {
	...
	async error(message: string, requestMaker: string, isAdmin = false) {
		Logger.error(
			`${
				requestMaker === undefined ? "Public" : requestMaker
			} : ${message}`
		);
		...
		else this.logfile("error", `(${requestMaker}) : ${message}`);
	}
	...
	async warn(message: string, requestMaker: string, isAdmin = false) {
		...
		else this.logfile("warning", `(${requestMaker}) : ${message}`);
	}
	...
	async log(message: string, requestMaker: string, isAdmin = false) {
		...
		else this.logfile("log", `(${requestMaker}) : ${message}`);
	}
	...
	async verbose(message: string, requestMaker: string) {
		...
		else this.logfile("verbose", `(${requestMaker}) : ${message}`);
	}
	...
	async logfile(type: string, message: string) {
		...
		if (today.getHours() < 10) hours = "0" + hours;
		if (today.getMinutes() < 10) minutes = "0" + minutes;
		if (today.getSeconds() < 10) seconds = "0" + seconds;
		const time = hours + ":" + minutes + ":" + seconds;
		const style = " [" + type.toUpperCase() + "]";
		fs.appendFile(
			file,
			time + style + " ".repeat(8 - type.length) + " > " + message + "\n",
			(err) => {
				if (err) throw err;
			}
		);
