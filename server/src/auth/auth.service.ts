						...
						.then(async (response) => {
							...
							if (response.data.cursus_users.length != 1) {
								for (
									...
									if (
										response.data.cursus_users[i].begin_at >
							...
							if (
								response.data.campus[
									response.data.campus.length - 1
								].city.toLowerCase() != "mulhouse"
							)
								clear = 2;
							else if (
								response.data.cursus_users[
									recent
								].cursus.kind.toLowerCase() == "piscine"
							)
								clear = 5;
							else clear = 7;
							let stud = {
							...
							if (!stud.img_small || stud.img_small === undefined)
								stud.img_small =
									"https://cdn.discordapp.com/attachments/907303542438629406/1040284333967163473/pas_de_pp.png";
							if (
								!stud.img_medium ||
								stud.img_medium === undefined
							)
								stud.img_medium =
									"https://cdn.discordapp.com/attachments/907303542438629406/1040284333967163473/pas_de_pp.png";
							const retStud = await this.studService.logUser(
								stud,
								"42intra-API"
							);
							return {
