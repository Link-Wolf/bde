...
export class GoodiesService {
	...
	async getThumbnail(id: number, login: any) {
		try {
			...
			if (!goodies) {
				this.logger.error(
					`Failed -> Get thumbnail of goodies ${id} : goodies doesn't exist`,
					login
				);
				throw new NotFoundException(
					`Failed to get thumbnail of goodies ${id} : goodies doesn't exist`
				);
			}
			const thumb_path = goodies.thumbnail_filename;
			if (!thumb_path) {
				this.logger.error(
					`Failed -> Get thumbnail of goodies ${id} : goodies doesn't has any thumbnail`,
					login
				);
				throw new NotFoundException(
					`Failed to get thumbnail of goodies ${id} : goodies doesn't has any thumbnail`
				);
			}
			...
			this.logger.error(
				`Failed -> Get thumbnail of goodies ${id} on database (${error})`,
				login
			);
			throw error;
		}
	...
	async getAlbum(id: number, login: any) {
		try {
			...
			if (!goodies || !fs.existsSync(goodies.album_path)) {
				this.logger.error(
					`Failed -> Get album of goodies ${id} : goodies doesn't exist`,
					login
				);
				throw new NotFoundException(
					`Failed to get album of goodies ${id}`
				);
			}
			...
			this.logger.error(
				`Failed -> Get album of goodies ${id} on database (${error})`,
				login
			);
			throw error;
		}
	...
	async saveAlbum(id: number, files: Array<Express.Multer.File>, login: any) {
		try {
				...
				for (let i = 0; i < files.length; i++) {
					fs.writeFile(
						...
						(err) => {
							if (err) {
								this.logger.error(
									`Failed -> Create goodies ${id} thumbnail (${err})`,
									login,
									true
								);
								throw err;
			...
			this.logger.error(
				`Failed -> Save thumbnail of goodies ${id} on database (${error})`,
				login,
				true
			);
			throw error;
	...
	resetAlbum(id: number, login: any) {
		try {
			...
			this.logger.error(
				`Failed -> Reset thumbnail of goodies ${id} on database (${error})`,
				login,
				true
			);
			throw error;
	...
	async saveThumbnail(id: number, file: Express.Multer.File, login: any) {
		try {
			...
			if (file === undefined) {
				...
				this.logger.log(
					`Saved thumbnail of goodies ${id}`,
					login,
					true
				);
			} else {
				path = `assets/thumbnails/goodies/${id}.${
	...
	async findAll(requestMaker: string): Promise<Goodies[]> {
		try {
			...
			this.logger.error(
				`Failed -> Get all goodies on database (${error})`,
				requestMaker
			);
			throw error;
		}
	...
	async findOne(id: number, requestMaker: string): Promise<Goodies> {
		try {
			...
			if (!goodies) {
				this.logger.error(
					`Failed -> Get goodies ${id} : goodies ${id} doesn't exist`,
					requestMaker
				);
				throw new NotFoundException(
					`Failed to get goodies ${id} : goodies ${id} doesn't exist`
				);
			}
			...
			this.logger.error(
				`Failed -> Get goodies ${id} on database (${error})`,
				requestMaker
			);
			throw error;
		}
	...
	async update(
		id: number,
		goodiesData: GoodiesDto,
		requestMaker: string
	): Promise<any> {
		try {
			if (!(await this.findOne(id, requestMaker))) {
				this.logger.error(
					`Failed -> Update goodies ${id} : goodies ${id} doesn't exist`,
					requestMaker,
					true
				);
				throw new NotFoundException(
					`Failed to update goodies ${id} : goodies doesn't exist`
				);
			}
			...
			this.logger.error(
				`Failed -> Update goodies ${id} on database (${error})`,
				requestMaker,
				true
			);
			throw error;
	...
	async create(goodiesDto: GoodiesDto, requestMaker: string): Promise<any> {
		try {
			...
			this.logger.warn(
				`Created new goodies ${goodiesDto.name}`,
				requestMaker,
				true
			);
			return ret;
			...
			this.logger.error(
				`Failed -> Create goodies ${goodiesDto.name} on database (${error})`,
				requestMaker,
				true
			);
			throw error;
	...
	async removeOne(id: number, requestMaker: string): Promise<any> {
		try {
			if (await this.findOne(id, requestMaker)) {
				...
				this.logger.warn(
					`Failed -> Delete goodies ${id} : goodies doesn't exist`,
					requestMaker,
					true
				);
				throw new NotFoundException(
					`Failed to delete goodies ${id} : goodies doesn't exist`
				);
			}
		} catch (error) {
			this.logger.error(
				`Failed to delete goodies ${id} on database(${error})`,
				requestMaker,
				true
			);
			throw error;
	...
	async removeAll(requestMaker: string): Promise<any> {
		try {
			...
			this.logger.error(
				`Failed -> Delete all goodies on database(${error})`,
				requestMaker,
				true
			);
			throw error;
