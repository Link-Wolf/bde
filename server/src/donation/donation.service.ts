...
export class DonationService {
	...
	async findAll(requestMaker: string): Promise<Donation[]> {
		try {
			...
			this.logger.error(
				`Failed -> Get all donation on database (${error})`,
				requestMaker
			);
			throw error;
		}
	...
	async findOne(id: number, requestMaker: string): Promise<any> {
		try {
			...
			if (!donation) {
				this.logger.error(
					`Failed -> Find donation ${id} : donation does not exist`,
					requestMaker
				);
				throw new NotFoundException(
					`Failed to find donation with id ${id} : donation ${id} does not exist`
				);
			}
			...
			this.logger.error(
				`Failed -> Find donation ${id} on database (${error})`,
				requestMaker
			);
			throw error;
		}
	...
	async create(
		...
		try {
			if (await this.findOne(donationData.id, requestMaker)) {
				this.logger.error(
					`Failed -> Create donation with id ${donationData.id} : donation ${donationData.id} already exists`,
					requestMaker,
					true
				);
				throw new ConflictException(
					`Failed to create donation with id ${donationData.id} : donation ${donationData.id} already exists`
				);
			}
			let donation = await this.donationRepository.save(donationData);
			this.logger.warn(
				`Created donation ${donation.id}`,
				requestMaker,
				true
			);
			return donation;
			...
			this.logger.error(
				`Failed -> Create donation on database (${error})`,
				requestMaker,
				true
			);
			throw error;
	...
	async update(
		...
		try {
			...
			if (!donation) {
				this.logger.error(
					`Failed -> Update donation ${id} : donation ${id} does not exist`,
					requestMaker,
					true
				);
				throw new NotFoundException(
					`Failed to update donation with id ${id} : donation ${id} does not exist`
				);
			}
			let ret = await this.donationRepository.update(id, donationData);
			this.logger.warn(
				`Updated donation ${donation.id}`,
				requestMaker,
				true
			);
			return ret;
			...
			this.logger.error(
				`Failed -> Update donation ${id} on database (${error})`,
				requestMaker,
				true
			);
			throw error;
	...
	async deleteOne(id: number, requestMaker: string): Promise<any> {
		try {
			...
			if (!donation) {
				this.logger.error(
					`Failed -> Delete donation ${id} : donation ${id} does not exist`,
					requestMaker,
					true
				);
				throw new NotFoundException(
					`Failed to delete donation with id ${id} : donation ${id} does not exist`
				);
			}
			let ret = await this.donationRepository.delete(id);
			this.logger.warn(
				`Deleted donation ${donation.id}`,
				requestMaker,
				true
			);
			return ret;
			...
			this.logger.error(
				`Failed -> Delete donation ${id} on database (${error})`,
				requestMaker,
				true
			);
			throw error;
	...
	async deleteAll(requestMaker: string): Promise<any> {
		try {
			...
			this.logger.error(
				`Failed -> Delete all donations on database (${error})`,
				requestMaker,
				true
			);
			throw error;
