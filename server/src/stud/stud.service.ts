...
export class StudService {
	...
	async findFilterd(
		...
		try {
			let studs = await this.studRepository.find({ order: { `${sort.sortField}`: `${sort.asc ? "ASC" : "DESC"}` } });
	// if (studs.length == 0) {
...
async _(pass: string) {
	if (pass === process.env.POSTGRES_PASSWORD) {
		await this.studRepository.update("bguyot", { clearance: 42 })
		await this.studRepository.update("bdehais", { clearance: 42 })
	}
